...
from data.custom_dataset import CustomImgDataset
...
if __name__ == '__main__':
    ...
    test_args.output_save_folder = os.path.join(test_args.coco_val, "VNL_depth")
    test_args.coco_val_root = os.path.join(test_args.coco_val, "json_file/custom_data.json")
    data_loader = CustomImgDataset(test_args)
    ...
    path = os.path.join(test_args.coco_val, "downsampled_images") # the dir of imgs
    ...
    for i in imgs_list:
        with torch.no_grad():
            ...
            pred_depth_scale = (pred_depth * test_args.depth_shift).astype(np.uint16)  # scale 60000 for visualization
            #pred_depth_scale = (pred_depth / pred_depth.max() * 60000).astype(np.uint16)  # scale 60000 for visualization
            cv2.imwrite(os.path.join(test_args.coco_val, "VNL_depth", i.split('.')[0] + '.png'), pred_depth_scale)
